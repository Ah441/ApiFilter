   ...
   applyFilters() {
    for (let param in this.params) {
        if (param.toLowerCase() !== 'sort' && param.toLowerCase() !== 'limit' && param.toLowerCase() !== 'offset' && param.toLowerCase() !== 'fields') {
            ...
            if (!this.objectsList.some(obj => obj.hasOwnProperty(param))) {
                this.objectsList = [{ error: `Unexistent field '${param}'` }];
                return;
            }
    ...
    for (let param in this.params) {
        ...
        switch (param.toLowerCase()) {
            case 'sort':
                ...
                if (!this.objectsList.some(obj => obj.hasOwnProperty(sortField))) {
                    this.objectsList = [{ error: "Unexistent field" }];
                    return;
                }
            ...
            case 'fields':
                ...
                for(let field of fieldsToShow) {
                    if (!this.objectsList.some(obj => obj.hasOwnProperty(field))) {
                        this.objectsList = [{ error: `Unexistent field '${field}'` }];
                        return;
                    }
                }
